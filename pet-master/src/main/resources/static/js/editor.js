...
function titleuploadImage() {
	...
	formData.append('boardType', boardName);
	...
	$.ajax({
		...
		success: function(data) {
			if (data) {
				...
				let imagePath = '/image_' + boardName +'/title_img/'  + imageFileName;
...
function uploadImage(event) {
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data) {
                let imagePath = '/image_' + boardName +'/temp_img/'  + data.imageFileName; // 이미지 저장 경로에 파일 이름을 추가합니다.  
                pasteHTML(imagePath);
            } else {
                alert("이미지 업로드에 실패했습니다.");
            }
