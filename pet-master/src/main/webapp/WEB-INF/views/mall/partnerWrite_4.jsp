...
<body id="page-top">
  ...
  <%@ include file="../include/mallHeader.jsp" %>
	...
	<input type="hidden" id="id" name="id" value="${partner_board.id}" readonly>
...
function save() {
    ...
    while (match = regex.exec(content)) {
        ...
        // content 변수 수정
        let pattern = new RegExp('(/image_mall/temp_img/' + imageName + ')', 'g');
        content = content.replace(pattern, '/image_mall/final_img/' + imageName); 
    }

    let requestData = {
        imageFileNames1: imageFileNames1,
        operationType: 'mall' // 또는 'other'
    };
    ...
    let finalImagePath = '/image_mall/title_img/' + titleimageName; // 최종 이미지 경로
...
function titleuploadImage(savePathType1) {
    return new Promise(function(resolve, reject) {
        ...
        let savePathType1 = 'mall'; // 변수를 정의한 다음에 사용합니다.
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    let imagePath = '/image_mall/title_img/'  + imageFileName;
...
function uploadImage(event) {
    ...
    let savePathType = "mall"; // 또는 필요에 따라 다른 값으로 설정
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                let imageFileName = response.imageFileName;
                let imagePath = '/image_mall/temp_img/'  + imageFileName; // 이미지 저장 경로에 파일 이름을 추가합니다.  
                pasteHTML(imagePath);
            } else {
